#let attachment_desc = [My *content*] 
// ####################
// # typst-letter-pro #
// ####################
// 
...
#let letter-generic(
  ...
  // Bottom matter
  if (type(attachments) == str ) { [#v(1fr) #attachment_desc: #attachments] }
  else if (type(attachments) == array and attachments.len() > 0 ) { [#v(1fr) #attachment_desc: #for atch in attachments [
    - #atch
  ]] }
}
...
/// - phone (content, none): Phone number of the sender
/// - email (content, none): Email of the sender
/// - extra (content, none): Extra information about the sender
#let header-simple(name, address, phone: none, email: none, extra: none) = {
  set text(size: 10pt)
...
#let letter-simple(
  ...
  if header == auto {
    header = pad(
      ...
      align(bottom + right, header-simple(
        ...
        email: sender.at("email", default: none),
  ...
  if (annotations == none or annotations == []) and (stamp == false) {
